    ...
    <div class="p-5 max-w-[1400px] mx-auto bg-gray-50 min-h-screen">
      ...
      <div class="flex-1 min-w-[240px]">
        <input
          ...
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#2C64F9] focus:border-transparent"
          @input="filterPeminjaman"
        />
      </div>
      </div>
  ...
  export default {
    ...
    // --- filter baru dengan prioritas + pencarian
    filterPeminjaman() {
        let data = [];
        if (this.filterStatus === 'all') data = [...this.peminjaman];
        else if (this.filterStatus === 'Dikembalikan-Terlambat')
          data = this.peminjaman.filter(
            (i) =>
              i.status === 'Dikembalikan' &&
              i.tanggal_dikembalikan &&
              new Date(i.tanggal_dikembalikan) > new Date(i.tanggal_kembali)
          );
        else if (this.filterStatus === 'Terlambat')
          data = this.peminjaman.filter((i) => i.status === 'Terlambat');
        else data = this.peminjaman.filter((i) => i.status === this.filterStatus);
        ...
        if (this.searchQuery.trim() !== '') {
          ...
          const activeStatuses = ['Pending', 'Approved', 'Dipinjam'];
          const active = data.filter((i) => activeStatuses.includes(i.status));
          const inactive = data.filter((i) => !activeStatuses.includes(i.status));

          this.filteredPeminjaman = [...active, ...inactive];
        },
