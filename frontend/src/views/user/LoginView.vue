...
export default {
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        // Sesuaikan URL backend-mu. Misal: port 5000 dan endpoint /api/login
        const response = await axios.post('http://localhost:5000/api/login', {
          username: this.username,
          password: this.password
        });

        // Jika login berhasil, simpan token & user info
        ...
        // Error dari server atau koneksi
        this.errorMessage = error.response?.data?.message || 'Login gagal. Periksa username dan password Anda.';
      } finally {
        this.loading = false;
      }
    },
  ...
  mounted() {
    ...
    if (token && user) {
      if (user.role === 'pustakawan') this.$router.push('/admin');
